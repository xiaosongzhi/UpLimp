#include "cboosttrain.h"
#include "ui_cboosttrain.h"
...
CBoostTrain::CBoostTrain(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CBoostTrain),
    isRecipeSelected(false)
{
    ...
    ui->level4_RadioButton->setVisible(false);
    ui->level5_RadioButton->setVisible(false);
}

CBoostTrain::~CBoostTrain()
{
    delete ui;
}

void CBoostTrain::loadStyleSheetFile()
{
    //加载样式文件s
...
ST_BoostParam CBoostTrain::getBoostGameParam()
{
    ST_BoostParam st_Param;
    st_Param.gameName = m_currentText;
    st_Param.trainMode = E_TRAINMODE_BOOST;
...
bool CBoostTrain::recipeIsSelected()
{
    return isRecipeSelected;
}

void CBoostTrain::setGameList(QList<ST_GameMsg>& gameList)
{
...
void CBoostTrain::slotGetCurrentText(QString text)
{
    m_currentText = text;
}
